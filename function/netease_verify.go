...
func (f *Function) ShowAuthServerError(neteaseRequireVerify bool, verifyURl string, errorInfo string) error {
	if !neteaseRequireVerify {
		...
		if err != nil {
			return fmt.Errorf("ShowAuthServerError: %v", err)
		}
		return nil
	}
	...
	_, _, err := f.interact.SendFormAndWaitResponse(form.ModalForm{
		...
		Contents: []form.ModalFormElement{
			form.ModalFormElementLabel{
				Text: "" +
					"操作 MC 账号时触发网易验证, 请§r§e复制§r下面输入框中的链接, 然后§r§e粘贴§r到浏览器打开以完成验证。" +
	...
	if err != nil {
		return fmt.Errorf("SendNeteaseVerifyInfo: %v", err)
	}
